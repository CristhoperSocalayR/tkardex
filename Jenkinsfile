pipeline {
    ...
    environment {
        ...
        PROJECT_KEY = 'CristhoperSocalayR_mastermicroservice'  // Nombre corregido del proyecto en SonarCloud
    ...
    stages {
        stage('Clone Repository') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/CristhoperSocalayR/tkardex.git'
            }
        }
        ...
        stage('Run Unit Tests') {
            steps {
                sh '''
                    mvn test \
                        -Dtest=MovementKardexServiceTest
                '''
            }
            post {
                always {
